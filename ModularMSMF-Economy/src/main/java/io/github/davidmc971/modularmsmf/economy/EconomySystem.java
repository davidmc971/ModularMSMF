 ...
 * 
 * @author Lightkeks with help from @author David Alexander Pfeiffer
 *         (davidmc971) TODO: changing all
...
public class EconomySystem implements IModularMSMFCommand {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
		...
		if (args.length == 0) {
			if (sender instanceof Player) {
				Utils.sendMessageWithConfiguredLanguage(sender, ChatFormat.INFO, "commands.eco.balance.self", "_value",
						(String) (getMoney(uuid) + currencyFormat));
			} else {
				Utils.sendMessageWithConfiguredLanguage(sender, ChatFormat.CONSOLE, "commands.eco.error.console");
			}
		} else if (args.length > 0) {
			switch (args[0].toLowerCase()) {
				case "help":
					if (args.length > 1) {
						Utils.sendMessageWithConfiguredLanguage(sender, ChatFormat.INFO, "commands.eco.helponly");
					} else {
						/**
				...
				case "take":
					// Argument checking
					if (args.length < 2) {
						Utils.sendMessageWithConfiguredLanguage(sender, ChatFormat.ERROR, "commands.eco.error.syntax");
						return true;
					...
					if (!(sender instanceof Player) && toSelf) {
						Utils.sendMessageWithConfiguredLanguage(sender, ChatFormat.CONSOLE,
					...
					// : language.getString("commands.eco.set.other.full").replace("_target",
					// args[1])) .replace("_value_old", before + currencyFormat)
					...
					if (sender instanceof ConsoleCommandSender) {
						...
						Utils.sendMessageWithConfiguredLanguage(sender, ChatFormat.INFO, "commands.eco.set.self.full",
								"_target", args[1], "_value_old", (String) (before + currencyFormat), "_value_new",
								(String) (getMoney(temp) + currencyFormat));
						// Utils.sendMessageWithConfiguredLanguage(plugin, Bukkit.getPlayer(target),
						// ChatFormat.INFO, "commands.eco.set.notify", "_sender", sender.getName(),
						// "_value_old", (String) (before+currencyFormat), "_value_new", (String)
						// (getMoney(temp)+currencyFormat));
					} else {
						Utils.sendMessageWithConfiguredLanguage(sender, ChatFormat.INFO, "commands.eco.set.other.full",
								"_target", args[1], "_value_old", (String) (before + currencyFormat), "_value_new",
								(String) (getMoney(temp) + currencyFormat));
						Utils.sendMessageWithConfiguredLanguage(Bukkit.getPlayer(target), ChatFormat.INFO,
								"commands.eco.set.notify", "_sender", sender.getName(), "_value_old",
								(String) (before + currencyFormat), "_value_new",
								(String) (getMoney(temp) + currencyFormat));
						// }
					}
					...
					// .replace("_sender", sender.getName()).replace("_value_old", before +
					// currencyFormat) .replace("_value_new", getMoney(temp) + currencyFormat));
					// Utils.sendMessageWithConfiguredLanguage(plugin, Bukkit.getPlayer(target),
					// ChatFormat.INFO, "commands.eco.set.notify", "_sender", sender.getName(),
					// "_value_old", (String) (before+currencyFormat), "_value_new", (String)
					// (getMoney(temp)+currencyFormat));
					...
					if (PermissionManager.checkPermission(sender, "eco_pay")) {
						...
						if (args.length == 3) {
							...
							if (amount <= 0) {
								...
								Utils.sendMessageWithConfiguredLanguage(Bukkit.getPlayer(target), ChatFormat.SUCCESS,
							...
							if (PermissionManager.checkPermission(sender, "eco_lookup")) {
								...
								if (target == null) {
									...
									Utils.sendMessageWithConfiguredLanguage(sender, ChatFormat.INFO,
											"commands.eco.balance.other", "_target", args[0], "_value",
											(String) (getMoney(uuid) + currencyFormat));
								}
								break;
							} else {
