...
public class EconomySystem implements IModularMSMFCommand {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
		...
		if (args.length == 0) {
			if (sender instanceof Player) {
				Utils.sendMessageWithConfiguredLanguage(plugin, sender, ChatFormat.INFO, "economy-module.commands.eco.balance.self", "_value", (String) (getMoney(uuid) + currencyFormat));
			} else {
				Utils.sendMessageWithConfiguredLanguage(plugin, sender, ChatFormat.CONSOLE, "economy-module.commands.eco.error.console");
			}
		} else if (args.length > 0) {
			switch (args[0].toLowerCase()) {
			case "help":
				if (args.length > 1) {
					Utils.sendMessageWithConfiguredLanguage(plugin, sender, ChatFormat.INFO, "economy-module.commands.eco.helponly");
				} else {
					/**
			...
			case "take":
				// Argument checking
				if (args.length < 2) {
					Utils.sendMessageWithConfiguredLanguage(plugin, sender, ChatFormat.ERROR, "economy-module.commands.eco.error.syntax");
					return true;
				...
				if (!toSelf) {
					if (target == null) {
						...
						if (p == null || !p.isOnline()) {
							...
							Utils.sendMessageWithConfiguredLanguage(plugin, sender, ChatFormat.ERROR, "coremodule.player.notonline");
							return true;
						}
					}
				}
				...
				if (toSelf = true){
					Utils.sendMessageWithConfiguredLanguage(plugin, sender, ChatFormat.INFO, "economy-module.commands.eco.set.self.full", "_target", args[1], "_value_old", (String) (before+currencyFormat), "_value_new", (String) (getMoney(temp)+currencyFormat));
					...
					Utils.sendMessageWithConfiguredLanguage(plugin, sender, ChatFormat.INFO, "economy-module.commands.eco.set.other.full", "_target", args[1], "_value_old", (String) (before+currencyFormat), "_value_new", (String) (getMoney(temp)+currencyFormat));
					Utils.sendMessageWithConfiguredLanguage(plugin, Bukkit.getPlayer(target), ChatFormat.INFO, "economy-module.commands.eco.set.notify", "_sender", sender.getName(), "_value_old", (String) (before+currencyFormat), "_value_new", (String) (getMoney(temp)+currencyFormat));
			//}	
				}
			...
			case "pay": //FIXME: Wichtig - Output = null, please fix
				if (PermissionManager.checkPermission(sender, "eco_pay")) {
					if (args.length < 2) {
						Utils.sendMessageWithConfiguredLanguage(plugin, sender, ChatFormat.ERROR, "economy-module.commands.money.missingamountandplayer");
					}
					if (args.length == 2) {
						Utils.sendMessageWithConfiguredLanguage(plugin, sender, ChatFormat.ERROR, "economy-module.commands.money.missingamount");
					}
					if (args.length == 3) {
						target = Utils.getPlayerUUIDByName(args[1]);
						if (target == null) {
						...
						try {
							...
							Utils.sendMessageWithConfiguredLanguage(plugin, sender, ChatFormat.ERROR, "economy-module.commands.money.amountonly");
							return true;
						}
						if (amount <= 0) {
							Utils.sendMessageWithConfiguredLanguage(plugin, sender, ChatFormat.ERROR, "economy-module.commands.money.invalidpayment");
						} else if (getMoney(uuid) - amount < 0) {
							Utils.sendMessageWithConfiguredLanguage(plugin, sender, ChatFormat.ERROR, "economy-module.commands.money.nomoneyleft");
						} else {
							takeMoney(uuid, amount);
							addMoney(target, amount);
							// sender.sendMessage("[Eco] Du hast " + args[1] + " " + amount + "$
							// überwiesen!"); //TODO: need to change language string
							Utils.sendMessageWithConfiguredLanguage(plugin, sender, ChatFormat.SUCCESS, "economy-module.commands.eco.paytoothers", "_target", args[1], "_amount", Double.toString(getMoney(target)));
							...
							Utils.sendMessageWithConfiguredLanguage(plugin, Bukkit.getPlayer(target), ChatFormat.SUCCESS, "economy-module.commands.eco.payfromothers", "_sender", sender.getName(), "_amount", Double.toString(getMoney(target)));
				...
				case 1:
					if (PermissionManager.checkPermission(sender, "eco_lookup")) {
						...
						if (target == null) {
							...
							Utils.sendMessageWithConfiguredLanguage(plugin, sender, ChatFormat.INFO, "economy-module.commands.eco.balance.other", "_target", args[0], "_value", (String) (getMoney(uuid) + currencyFormat));
						}
						break;
					} else {
						Utils.sendMessageWithConfiguredLanguage(plugin, sender, ChatFormat.NOPERM, "coremodule.player.nopermission");
